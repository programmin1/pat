...
function initConnectModal() {
    ...
    $('#AllFormsList').html('');
    $.each(winforms, function(lbl, url) {
        $('#AllFormsList').append('<li><a target="_blank" href="' + url + '">' + lbl + '</a></li>');
    });
    $('#templatechosen_btn').click(function() {

    });
    window.addEventListener('message', function(e) {
        //post back data
        console.log(event.data)
        $('#templatechooser').modal('hide');
        $('#msg_body').val(event.data.Msg);
        $('#msg_subject').val(event.data.Subject);
        if (event.data.To) jQuery('#msg_to-tokenfield').val(event.data.To);
    });
